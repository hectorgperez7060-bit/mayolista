...
import {
  ...
  setVendorBlocked, markOrderSeen, markOrderProcessed, deleteOrder,
...
export default function AdminPanelScreen({ onNavigate }) {
  ...
  const markActioned = (pedidoId) => setActionedOrders(prev => ({ ...prev, [pedidoId]: true }));
  ...
  const handleShareClientCode = async () => {
  ...
  const handleManualPurge = async () => {
    if (!window.confirm('¿Borrar TODOS los productos de la nube? No se puede deshacer.')) return;
    setSyncing(true); setSyncMsg('Limpiando...');
    try {
      await clearProductosFirebase(empresaId, count => setSyncMsg(`Limpiando (${count} eliminados)...`));
      setSyncMsg('✓ Base de datos limpia');
    } catch (err) { window.alert('Error al limpiar: ' + err.message); }
    setSyncing(false);
    setTimeout(() => setSyncMsg(''), 3500);
  };
  ...
  const handleProcess = async (pedidoId) => {
    try {
      await markOrderProcessed(empresaId, pedidoId);
      if (expandedId === pedidoId) setExpandedId(null);
    } catch {}
  };
  ...
  const activePedidos     = pedidos.filter(p => p.estado !== 'procesado');
  const historialPedidos  = pedidos.filter(p => p.estado === 'procesado');
  const pendientes        = activePedidos.filter(p => p.estado === 'pendiente').length;

  // ── Shared icon button style ──
  const iconBtn = (color) => ({
    background: 'none', border: 'none', cursor: 'pointer',
    color: color || 'var(--text-muted)', padding: '0.3rem',
  ...
  return (
    ...
    <div className="p-4" style={{ paddingBottom: '2rem' }}>
      ...
      {/* ── Header ── */}
      <header className="mb-3" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        <button onClick={() => onNavigate('home')} style={iconBtn()}>
          <ChevronLeft size={24} />
        </button>
        <div style={{ flex: 1 }}>
          <h1 className="text-xl">Panel de empresa</h1>
          <p className="text-muted" style={{ fontSize: '0.82rem' }}>{mayorista.nombre || 'Mi empresa'}</p>
        </div>
      </header>

      {/* ── Códigos de acceso (compacto, ambos juntos) ── */}
      <div className="glass-panel p-3 mb-3">
        <p style={{ fontSize: '0.68rem', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.08em', marginBottom: '10px' }}>
          CÓDIGOS DE ACCESO
        </p>

        {/* Fila vendedores */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <div style={{ width: '72px', fontSize: '0.7rem', fontWeight: 700, color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: '3px', flexShrink: 0 }}>
            <Users size={11} /> Vendedores
          </div>
          <span style={{
            flex: 1, fontFamily: 'monospace', fontWeight: 800, fontSize: '1rem',
            letterSpacing: '0.1em', color: 'var(--primary)'
          }}>
            {empresaCodigo}
          </span>
          <button onClick={handleCopyCode} style={iconBtn(copied ? 'hsl(150,80%,50%)' : undefined)} title="Copiar">
            {copied ? <Check size={16} /> : <Copy size={16} />}
          </button>
          <button onClick={handleShareCode} style={iconBtn()} title="Compartir">
            <Share2 size={16} />
          </button>
          {!confirmNewCode ? (
            <button onClick={() => setConfirmNewCode(true)} style={iconBtn('hsl(0,80%,65%)')} title="Generar nuevo">
              <RefreshCw size={14} />
            </button>
          ) : (
            <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
              <span style={{ fontSize: '0.72rem', color: 'hsl(0,80%,65%)' }}>¿Seguro?</span>
              <button onClick={handleNewCode} disabled={generatingCode} style={{
                padding: '2px 8px', borderRadius: '6px', border: 'none', cursor: 'pointer',
                background: 'hsl(0,80%,55%)', color: '#fff', fontWeight: 700, fontSize: '0.72rem'
              }}>
                {generatingCode ? <Loader size={11} style={{ animation: 'spin 1s linear infinite' }} /> : 'Sí'}
              </button>
              <button onClick={() => setConfirmNewCode(false)} style={{
                padding: '2px 6px', borderRadius: '6px', border: '1px solid var(--border-color)',
                background: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: '0.72rem'
              }}>No</button>
            </div>
          )}
        </div>
        ...
        {/* Fila clientes */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <div style={{ width: '72px', fontSize: '0.7rem', fontWeight: 700, color: 'hsl(35,100%,60%)', display: 'flex', alignItems: 'center', gap: '3px', flexShrink: 0 }}>
            <Users size={11} /> Clientes
          </div>
          <span style={{
            flex: 1, fontFamily: 'monospace', fontWeight: 800, fontSize: '1rem',
            letterSpacing: '0.1em', color: 'hsl(35,100%,60%)'
          }}>
            {codigoClientesLocal || '—'}
          </span>
          ...
          }} style={iconBtn(codigoClientesCopied ? 'hsl(150,80%,50%)' : undefined)} title="Copiar">
            {codigoClientesCopied ? <Check size={16} /> : <Copy size={16} />}
          </button>
          <button onClick={handleShareClientCode} style={iconBtn()} title="Compartir">
            <Share2 size={16} />
          </button>
          <button onClick={async () => {
            if (!codigoClientesLocal) {
              setRegenLoading(true);
              try { const n = await regenerarCodigoClientes(empresaId); setCodigoClientesLocal(n); } catch {}
              setRegenLoading(false);
              return;
            }
            if (!confirm('¿Regenerar el código de clientes? El anterior dejará de funcionar.')) return;
            setRegenLoading(true);
            try { const n = await regenerarCodigoClientes(empresaId); setCodigoClientesLocal(n); } catch {}
            setRegenLoading(false);
          }} disabled={regenLoading} style={iconBtn(codigoClientesLocal ? 'hsl(0,80%,65%)' : 'hsl(35,100%,60%)')}
            title={codigoClientesLocal ? 'Regenerar' : 'Generar código'}>
            <RefreshCw size={14} style={regenLoading ? { animation: 'spin 1s linear infinite' } : {}} />
          </button>
        </div>
      </div>

      {/* ── Publicar lista (barra compacta) ── */}
      <div className="glass-panel p-3 mb-3">
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          <button onClick={handleSyncProducts} disabled={syncing} style={{
            flex: 1, padding: '0.6rem 0.75rem', borderRadius: '10px', border: 'none',
            ...
            {syncing && syncProgress
              ? <><Loader size={13} style={{ animation: 'spin 1s linear infinite' }} /> {syncProgress.current}/{syncProgress.total}</>
              : `Publicar lista (${products.length})`}
          </button>
          <button onClick={handleSyncData} disabled={syncing} title="Publicar datos de empresa" style={{
          ...
          <button onClick={handleManualPurge} disabled={syncing} title="Borrar productos de la nube" style={{
            padding: '0.6rem 0.75rem', borderRadius: '10px',
            border: '1px solid hsla(0,80%,55%,0.35)', background: 'hsla(0,80%,55%,0.08)',
            color: 'hsl(0,80%,65%)', cursor: syncing ? 'default' : 'pointer',
            opacity: syncing ? 0.7 : 1, display: 'flex', alignItems: 'center'
          }}>
            <Trash2 size={15} />
          </button>
        </div>
        {syncMsg && (
          <p style={{ marginTop: '6px', fontSize: '0.78rem', fontWeight: 600,
      ...
      <div style={{ display: 'flex', gap: '0.4rem', marginBottom: '1rem' }}>
        {[
          ...
          { key: 'promos',    label: 'Promos',    icon: <Tag size={14} /> },
        ].map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} style={{
            flex: 1, padding: '0.55rem 0.5rem', borderRadius: '12px', border: 'none', cursor: 'pointer',
            ...
            {t.key === 'pedidos' && pendientes > 0 && (
              <span style={{
                ...
                borderRadius: '999px', padding: '1px 6px', fontSize: '0.7rem', lineHeight: 1.4
              }}>{pendientes}</span>
            )}
          </button>
        ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          {activePedidos.length > 0 && (
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '0.25rem' }}>
              <button onClick={() => exportTodosExcel(activePedidos)} style={{
          ...
          {activePedidos.length === 0 ? (
            <div className="glass-panel p-5" style={{ textAlign: 'center', color: 'var(--text-muted)' }}>
              ...
              {historialPedidos.length > 0 && <p style={{ fontSize: '0.8rem', marginTop: '0.5rem' }}>Ver pedidos procesados en Historial →</p>}
            </div>
          ) : (() => {
            const grouped = {};
                      ...
                      {isOpen && (
                        <div style={{ marginTop: '10px', borderTop: '1px solid var(--border-color)', paddingTop: '10px' }}>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '0.4rem', marginTop: '10px' }}>
                            <button onClick={() => { window.open(`https://wa.me/?text=${encodeURIComponent(buildOrderText(p))}`, '_blank'); markActioned(p.id); }}
                            ...
                            <button onClick={() => { exportPedidoPDF(p, mayorista); markActioned(p.id); }}
                            ...
                            <button onClick={() => { exportPedidoExcel(p); markActioned(p.id); }}
                            ...
                            <button onClick={() => { printPedido(p, mayorista); markActioned(p.id); }}
                          ...
                          {/* ── Procesar pedido ── */}
                          {!actionedOrders[p.id] && (
                            <p style={{ textAlign: 'center', fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '8px', opacity: 0.7 }}>
                              Compartir o imprimir para habilitar
                            </p>
                          )}
                          <button
                            onClick={() => actionedOrders[p.id] && handleProcess(p.id)}
                            disabled={!actionedOrders[p.id]}
                            style={{
                              width: '100%', marginTop: '4px',
                              padding: '0.6rem',
                              borderRadius: '12px', border: 'none',
                              cursor: actionedOrders[p.id] ? 'pointer' : 'not-allowed',
                              background: actionedOrders[p.id]
                                ? 'linear-gradient(135deg, hsl(150,75%,35%), hsl(160,70%,30%))'
                                : 'var(--bg-surface-glass)',
                              color: actionedOrders[p.id] ? '#fff' : 'var(--text-muted)',
                              fontWeight: 800, fontSize: '0.82rem',
                              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px',
                              boxShadow: actionedOrders[p.id] ? '0 4px 14px hsla(150,70%,35%,0.35)' : 'none',
                              opacity: actionedOrders[p.id] ? 1 : 0.5,
                              transition: 'all 0.2s ease'
                            }}
                          >
                            <Check size={15} /> Procesar pedido · Mandar al historial
                          </button>

                          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '6px' }}>
                            {isConfirmingDelete ? (
                              <div style={{ display: 'flex', gap: '0.4rem' }}>
                                <button onClick={() => handleDelete(p.id)} style={{ padding: '0.35rem 0.75rem', borderRadius: '10px', border: 'none', background: 'hsl(0,80%,55%)', color: '#fff', fontWeight: 700, fontSize: '0.78rem', cursor: 'pointer' }}>Cancelar pedido</button>
      ...
      {/* ══════════════════════════════════
          TAB: EQUIPO (Vendedores + Clientes)
      ══════════════════════════════════ */}
      {tab === 'equipo' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2px' }}>
            <p style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.08em' }}>
              VENDEDORES ({otrosVendedores.length})
            </p>
          </div>
          ...
          {otrosVendedores.length === 0 ? (
            <div className="glass-panel p-4" style={{ textAlign: 'center', color: 'var(--text-muted)' }}>
              ...
              <p style={{ fontSize: '0.88rem' }}>Ningún vendedor conectado</p>
              <p style={{ fontSize: '0.75rem', marginTop: '3px' }}>Compartí el código de vendedores para que se unan</p>
            </div>
            ...
            <div key={v.id} className="glass-panel p-3" style={{ opacity: v.bloqueado ? 0.6 : 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <button onClick={() => setVendorBlocked(empresaId, v.id, !v.bloqueado)} style={{
                  padding: '0.35rem 0.65rem', borderRadius: '10px', border: 'none', cursor: 'pointer',
                  background: v.bloqueado ? 'hsla(150,80%,40%,0.15)' : 'hsla(0,80%,55%,0.12)',
                  color: v.bloqueado ? 'hsl(150,80%,50%)' : 'hsl(0,80%,65%)',
                  fontWeight: 700, fontSize: '0.75rem',
                  display: 'flex', alignItems: 'center', gap: '4px'
                }}>
                  {v.bloqueado ? <><Unlock size={13} /> Activar</> : <><Lock size={13} /> Bloquear</>}
                </button>
                <button onClick={() => setConfirmRemoveVendorId(v.id)} style={{
                  padding: '0.35rem 0.45rem', borderRadius: '10px', border: 'none', cursor: 'pointer',
          ...
          {/* Divider */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', margin: '0.5rem 0 2px' }}>
            <div style={{ flex: 1, height: '1px', background: 'var(--border-color)' }} />
            <p style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>
              IMPORTAR CLIENTES
            </p>
            <div style={{ flex: 1, height: '1px', background: 'var(--border-color)' }} />
          </div>
          ...
          <div className="glass-panel p-4">
            <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginBottom: '0.85rem' }}>
              Columnas requeridas: <strong>nombre</strong>, <strong>cuit</strong>. Opcionales: condicionIVA, direccion, localidad, zona, telefono, email, id, codigoCliente.
              Si el cliente ya existe por CUIT o ID, se actualiza sin borrar su historial.
            </p>
            <label style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.6rem',
              padding: '0.9rem', borderRadius: '12px', border: '2px dashed var(--border-color)',
              cursor: 'pointer', color: 'var(--text-muted)', fontWeight: 600, fontSize: '0.9rem'
            }}>
              <Upload size={18} />
            ...
            {clientImportMsg && (
              <p style={{ marginTop: '0.75rem', fontSize: '0.85rem', fontWeight: 600,
                color: clientImportMsg.startsWith('✓') ? 'hsl(150,80%,55%)' : 'var(--danger)' }}>
                {clientImportMsg}
              </p>
            )}
          </div>
      ...
      {tab === 'promos' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <p style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.08em' }}>
              OFERTAS DEL DÍA ({ofertas.length})
            </p>
            <button onClick={() => setOfertaForm({ productId: '', productCode: '', productName: '', productPrice: '', condicionTipo: 'percent_qty', condicionCantidad: '', condicionValor: '' })} style={{
              padding: '0.35rem 0.8rem', borderRadius: '10px', border: 'none',
              background: 'hsl(35,100%,50%)', color: '#fff',
          ...
          {ofertaForm && (() => {
            ...
            const sugeridos = busqueda.length >= 2 ? searchProducts(products, busqueda).slice(0, 15) : [];
            const productoSeleccionado = !!ofertaForm.productId;
            return (
              <div className="glass-panel p-4" style={{ borderColor: 'hsla(35,100%,50%,0.4)' }}>
                <p style={{ fontWeight: 700, marginBottom: '0.75rem', fontSize: '0.92rem' }}>Nueva oferta</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                  {!productoSeleccionado ? (
                    <div style={{ position: 'relative' }}>
                      <input className="input-glass" placeholder="Buscá el producto..."
                        value={busqueda}
                        ...
                        autoFocus />
                      {sugeridos.length > 0 && (
                        <div onTouchStart={e => e.stopPropagation()} style={{
                          position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50,
                          background: 'var(--bg-surface)', border: '1px solid var(--border-color)',
                          borderRadius: '10px', marginTop: '4px', maxHeight: '240px', overflowY: 'auto',
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.65rem 1rem', background: 'hsla(35,100%,50%,0.1)', borderRadius: '10px', border: '1px solid hsla(35,100%,50%,0.3)' }}>
                      <div style={{ flex: 1 }}>
                        <p style={{ fontWeight: 700, fontSize: '0.92rem' }}>{ofertaForm.productName}</p>
                  ...
                  {productoSeleccionado && (<>
                    <select className="input-glass" value={ofertaForm.condicionTipo}
                      ...
                      <option value="percent_qty">% de descuento al comprar X o más</option>
                      <option value="bonus_qty">Comprando X unidades, N de regalo</option>
                    </select>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                      <input className="input-glass" type="number" placeholder={ofertaForm.condicionTipo === 'percent_qty' ? 'Cantidad mínima' : 'Comprando X'}
                        value={ofertaForm.condicionCantidad}
                        onChange={e => setOfertaForm(f => ({ ...f, condicionCantidad: e.target.value }))}
                        style={{ fontSize: '0.9rem', padding: '0.65rem' }} />
                      <input className="input-glass" type="number" placeholder={ofertaForm.condicionTipo === 'percent_qty' ? '% descuento' : 'Unidades regalo'}
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button onClick={async () => {
                      ...
                      await saveOferta(empresaId, {
                        productId: ofertaForm.productId, productCode: ofertaForm.productCode,
                        productName: ofertaForm.productName, productPrice: Number(ofertaForm.productPrice) || 0,
                        condicionTipo: ofertaForm.condicionTipo,
                        condicionCantidad: Number(ofertaForm.condicionCantidad),
                        condicionValor: Number(ofertaForm.condicionValor), activa: true
                      });
                      setOfertaForm(null); setOfertaSaving(false);
                      ...
                      color: productoSeleccionado ? '#fff' : 'var(--text-muted)', fontWeight: 700,
                      cursor: productoSeleccionado ? 'pointer' : 'default'
                    }}>
                      {ofertaSaving ? 'Guardando...' : 'Guardar oferta'}
                    </button>
                    <button onClick={() => setOfertaForm(null)} style={{
                      padding: '0.7rem', borderRadius: '10px', border: '1px solid var(--border-color)',
          ...
          {ofertas.length === 0 && !ofertaForm && (
            <div className="glass-panel p-4" style={{ textAlign: 'center', color: 'var(--text-muted)' }}>
              <Tag size={28} style={{ opacity: 0.25, marginBottom: '0.5rem' }} />
              <p style={{ fontSize: '0.88rem' }}>No hay ofertas activas</p>
            </div>
          )}
          ...
          {ofertas.map(of => (
            <div key={of.id} className="glass-panel p-3" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontSize: '0.75rem', color: 'hsl(35,100%,60%)', fontWeight: 600, marginTop: '2px' }}>
              ...
              <button onClick={() => deleteOferta(empresaId, of.id)} style={iconBtn('var(--danger)')}>
                <Trash2 size={15} />
              </button>
            </div>
          ))}
          ...
          <div className="glass-panel p-4">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                <input className="input-glass" type="number" min="1" max="100" placeholder="% de descuento"
              ...
              <button onClick={async () => {
                ...
                background: 'hsl(35,100%,50%)', color: '#fff', fontWeight: 700, cursor: 'pointer'
              }}>
              ...
              {genResult && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '0.75rem', background: 'hsla(150,80%,40%,0.12)', borderRadius: '10px', border: '1px solid hsla(150,80%,40%,0.3)' }}>
                  <Check size={15} style={{ color: 'hsl(150,80%,55%)' }} />
                  <p style={{ fontWeight: 800, fontSize: '1rem', fontFamily: 'monospace', color: 'hsl(150,80%,60%)', flex: 1 }}>{genResult}</p>
                  <button onClick={() => navigator.clipboard.writeText(genResult)} style={iconBtn()}>
                    <Copy size={15} />
                  </button>
                </div>
              )}
